...
import { Clock, AlignLeft, Trash2, ChevronLeft, ChevronRight, Calendar as CalendarIcon } from "lucide-react"
...
export default function TimeTracker() {
  ...
  const isSlotOccupied = (slot: number, excludeTaskId?: string) => {
    return filteredTasks.some((task) => task.id !== excludeTaskId && slot >= task.startSlot && slot < task.endSlot)
  }
  ...
  const handleMouseDown = (e: React.MouseEvent, slot: number) => {
    ...
    const clickedTask = filteredTasks.find((task) => slot >= task.startSlot && slot < task.endSlot)
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if ((dragOperation.type === 'click' && distance > 5) ||
      (dragOperation.type === 'drag' && currentSlot !== dragOperation.currentSlot)) {
  ...
  const handleMouseUp = () => {
    ...
    // Handle drag operation
      ...
      if (dragOperation.resizingTask) {
        ...
        setTasks((prev) =>
          prev.map((task) => {
            ...
            if (dragOperation.resizeType === 'start') {
              const newStartSlot = Math.min(dragOperation.currentSlot, task.endSlot - 1)
              ...
              while (adjustedStartSlot < task.endSlot - 1 && isSlotOccupied(adjustedStartSlot, task.id)) {
                adjustedStartSlot++
              }

              return { ...task, startSlot: adjustedStartSlot }
            } else {
              const newEndSlot = Math.max(dragOperation.currentSlot + 1, task.startSlot + 1)
              ...
              while (adjustedEndSlot <= finalEndSlot && !isSlotOccupied(adjustedEndSlot - 1, task.id)) {
                adjustedEndSlot++
              }
        ...
        // Create new task
        const startSlot = Math.min(dragOperation.startPosition.slot, dragOperation.currentSlot)
        const endSlot = Math.max(dragOperation.startPosition.slot, dragOperation.currentSlot) + 1

        // Check if any slots are occupied
        const hasConflict = Array.from({ length: endSlot - startSlot }, (_, i) => startSlot + i).some((slot) =>
          isSlotOccupied(slot),
        )
  ...
  const getPreviewSlots = () => {
    ...
    } else if (dragOperation.type === 'drag' && dragOperation.resizingTask && dragOperation.resizeType) {
  ...
  // Focus on title input when a task is selected
  useEffect(() => {
      ...
      <div className="flex-1 p-4 overflow-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-3xl font-bold">{format(selectedDate, "MMMM d, yyyy")}</h1>
            <Popover>
        ...
        <Card className="p-6">
          ...
          >
            ...
            {timeSlots.map((time, index) => (
              ...
              >
                <div className="w-20 text-sm text-gray-600 py-2 px-2 border-r border-gray-200 bg-gray-100">{time}</div>
                <div className="flex-1 relative">
                  {/* Preview for new task or resizing */}
                  {previewSlots && index >= previewSlots.startSlot && index < previewSlots.endSlot && (
            ...
            {filteredTasks.map((task) => (
              <div
                key={task.id}
                className={`absolute left-20 right-0 border-2 rounded cursor-move z-20 ${task.color} ${selectedTaskId === task.id ? "ring-2 ring-blue-500" : ""
                  }`}
                style={{
              ...
              >
                <div className="p-2 h-full overflow-hidden">
                  ...
                  <div className="text-xs text-gray-600 mt-1">
                    {formatTime(task.startSlot)} - {formatTime(task.endSlot)}
                  </div>
                  {task.description && (
                    <div className="text-xs text-gray-700 mt-1 line-clamp-2">{task.description}</div>
                  )}
                </div>
              </div>
            ))}
